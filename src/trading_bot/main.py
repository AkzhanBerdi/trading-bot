...
class TradingBot:
    ...
    async def initialize_grids(self):
        ...
        try:
            ...
            if ada_price and avax_price:
                ...
                self.logger.info(f"Grids initialized: ADA @ ${ada_price:.4f}, AVAX @ ${avax_price:.4f}")
    ...
    async def check_grid_strategies(self):
        ...
        try:
            ...
            if ada_price:
                ...
                if ada_signals:
                    ...
                    for signal in ada_signals:
                        self.logger.info(f"   📍 Signal: {signal['action']} level {signal['level']} "
                        ...
                        if signal['signal_strength'] >= 0.7:
                            ...
                            if success:
                                self.daily_trades += 1
                                self.logger.info(f"✅ ADA grid trade executed: {signal['action']} at ${signal['price']:.4f}")
                else:
                ...
                if avax_signals:
                    ...
                    for signal in avax_signals:
                        ...
                        if signal['signal_strength'] >= 0.7:
                            ...
                            if success:
                                self.daily_trades += 1
                                self.logger.info(f"✅ AVAX grid trade executed: {signal['action']} at ${signal['price']:.4f}")
                else:
            ...
            await telegram_notifier.notify_trade_error(
                symbol="GRID_SYSTEM",
                action="CHECK_STRATEGIES",
                error_message=str(e)
            )

    async def execute_grid_order_with_notifications(self, grid_trader, signal, asset_name):
        ...
        try:
            ...
            if order and order.get('status') == 'FILLED':
                ...
                filled_price = float(order.get('fills', [{}])[0].get('price', signal['price']))
                ...
                self.logger.info(f"Grid order executed: {action} {filled_quantity} {symbol} at level {signal['level']}")
                return True
                
            else:
                # Order failed
                error_msg = order.get('msg', 'Unknown order error') if order else 'Order returned None'
    ...
    def _log_grid_details(self, asset_name: str, grid_trader, current_price: float):
        ...
        filled_buy_levels = [o['level'] for o in grid_trader.filled_orders if o['side'] == 'BUY']
        filled_sell_levels = [o['level'] for o in grid_trader.filled_orders if o['side'] == 'SELL']
        ...
        for level in grid_trader.buy_levels:
            ...
            if distance < nearest_buy_distance and level['level'] not in filled_buy_levels:
        ...
        for level in grid_trader.sell_levels:
            ...
            if distance < nearest_sell_distance and level['level'] not in filled_sell_levels:
                nearest_sell_distance = distance
                nearest_sell_price = level['price']
        
        # Log summary of nearest levels
        if nearest_buy_price:
            self.logger.info(f"   🎯 {asset_name} Nearest BUY: ${nearest_buy_price:.4f} "
                            f"(${nearest_buy_distance:.4f} below current)")
        
        if nearest_sell_price:
            self.logger.info(f"   🎯 {asset_name} Nearest SELL: ${nearest_sell_price:.4f} "
        ...
        self.logger.info(f"   📊 {asset_name} Grid Status: "
    ...
    async def send_portfolio_update(self):
        ...
        try:
            ...
            # Sort by value
            top_assets = dict(sorted(top_assets.items(), key=lambda x: x[1], reverse=True))
    ...
    async def run_cycle(self):
        ...
        try:
            ...
            self.logger.info(f"ADA Grid: {ada_status['total_orders']} orders, "
                           f"${ada_status['total_volume_usdt']:.2f} volume")
            self.logger.info(f"AVAX Grid: {avax_status['total_orders']} orders, "
                           f"${avax_status['total_volume_usdt']:.2f} volume")

        except Exception as e:
            self.logger.error(f"Error in trading cycle: {e}")
            await telegram_notifier.notify_bot_status("error", f"Trading cycle error: {e}")
    ...
    async def run(self):
        ...
        if telegram_notifier.enabled:
            await telegram_notifier.notify_bot_status("started", 
            ...
            self.logger.warning("📴 Telegram notifications disabled - check credentials in .env")

        # Test connection
        if not self.binance.test_connection():
            self.logger.error("❌ Binance connection failed, exiting")
            if telegram_notifier.enabled:
                await telegram_notifier.notify_bot_status("error", "Binance connection failed - bot stopping")
            return
        ...
        try:
            while self.running:
                ...
                if cycle_count % 20 == 0 and telegram_notifier.enabled:
            ...
            if telegram_notifier.enabled:
                await telegram_notifier.notify_bot_status("stopped", "Bot shutdown requested by user")
        except Exception as e:
            self.logger.error(f"❌ Unexpected error: {e}")
            if telegram_notifier.enabled:
                await telegram_notifier.notify_bot_status("error", f"Unexpected error: {e}")
        finally:
            self.running = False
            if telegram_notifier.enabled:
                await telegram_notifier.notify_bot_status("stopped", "Trading bot has stopped")
...
def main():
    ...
    try:
        ...
        try:
            ...
            if loop.is_running():
                asyncio.create_task(telegram_notifier.notify_bot_status("error", f"Failed to start bot: {e}"))
        except:
